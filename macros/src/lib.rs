...
pub fn derive_row(item: TokenStream) -> TokenStream {
    ...
    let parsed_struct = match venial::parse_item(proc_macro2::TokenStream::from(item)) {
        Ok(venial::Item::Struct(s)) => s,
        Ok(_) => {
            panic!(concat!("Can only use ", "Row derive macro on structs"))
        }
        Err(_) => {
            panic!("Parsing error")
        }
...
fn generate_table_row_impl(
    ...
    let field_types_fn_definition = generate_field_types_fn_definition(parsed_fields);
...
fn generate_table_row_setup_fn_definition(
    ...
    let mut fields_setup_sql = proc_macro2::TokenStream::new();
...
fn generate_tabled_fn_definitions(fields: &venial::NamedFields) -> proc_macro2::TokenStream {
    ...
    for (field, _) in fields.fields.iter() {
        fields_args.extend(
            format!("\"{}\", ", field.name.to_string()).parse::<proc_macro2::TokenStream>(),
        );
    }
...
fn generate_field_types_fn_definition(fields: &venial::NamedFields) -> proc_macro2::TokenStream {
    ...
    quote::quote!(
        fn field_types() -> Vec<reliquary::db::FieldTypeInfo> {
            vec![#field_types]
        }
    )
}

fn generate_get_fields_as_strings_fn_definition(
    fields: &venial::NamedFields,
) -> proc_macro2::TokenStream {
            ...
            if let Some(segment) = attribute.get_single_path_segment() {
                if segment == "display_table" {
                    ...
                    if value_tokens.len() == 3 {
                        display_table =
                            Some((value_tokens[0].to_string(), value_tokens[2].to_string()));
                    }
                }
            }
        }
        ...
        let get_field = quote::quote! {
            {
                ...
                let display_str_args
                    = reliquary::db::TableFieldDisplayStringArgs {
                        db_connection,
